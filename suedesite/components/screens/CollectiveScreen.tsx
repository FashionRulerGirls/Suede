...
import { SectionHeading, MeasurementSpec, Badge, Button, Avatar, Icon } from '@/components/ds';
...
export function CollectiveScreen({ onRoute, authed = false }: any) {
  ...
  React.useEffect(() => {
    if (!real || !user) { setRealMembers(null); return; }
    let alive = true;
    const sb = createClient();
    if (!sb) return;
    (async () => {
  ...
  // Real users see the live directory (may be empty until members opt in).
  if (real) {
